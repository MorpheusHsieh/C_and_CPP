...
void main(void)
{
  ...
  for (x=0; x<size2; x++)
    for (y=0; y<size2; y++)
      bk2[x+1][y+1]=(data2[x*size2+y]-128);
  Fast_DCT(bk2,size2,BS,1);
  ...
  for (int i=0; i<bknum; i++)
  {
    ...
    for (x=0; x<BS; x++)
      for (y=0; y<BS; y++)
        data3[X1*BS*size1+Y1*BS+x*size1+y] =
        (unsigned char)(bk1[X2*BS+x+1][Y2*BS+y+1] + 128.0);
  }
  delete [] data1;
  delete [] data2;
