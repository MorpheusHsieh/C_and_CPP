...
void main(void)
{
   ...
   // Write Meta prefix of BlackWhite picture
   META_PREFIX prefixBW;
   fread(&prefixBW, sizeof(META_PREFIX),1,fin1);

   // Write Meta prefix
   META_PREFIX prefix;
   fread(&prefix, sizeof(META_PREFIX), 1, fin2);

   prefix.reserved[0] = prefixBW.width;
   prefix.reserved[1] = byte(prefixBW.width>>0x08);
   prefix.reserved[2] = prefixBW.height;
   prefix.reserved[3] = byte(prefixBW.height>>0x08);
