...
//----------------------------------------------------------------------
#define myfunc 0.5*q(y*sqrt(aaa*Eb_N0)*cos(x))*rician_pdf(kk,y)*theta_pdf(bb*Eb_N0,x)
  // below 3 equations are for Rayleigh pdf:
  //#define myfunc 0.5*q(y*sqrt(aaa*Eb_N0))*rician_pdf(10,y)
  //#define yy1_x   0
  //#define yy2_x   3
  //#define X_1  0
  //#define X_2  1
...
int step_number=5;
double Eb_N0=120.0;
double aaa=1.38,bb=0,kk=0;
double bbb[]={5,10,15,20};
double kkk[]={0,4,8};
double sum=0.0;
...
void main()
{
 ...
 if ((out0 = fopen("d:\\trash\\pa50.dat", "a+t")) == NULL)
 {
   fprintf(stderr, "Cannot open input file.\n");
   return 1;
 }
 if ((out1 = fopen("d:\\trash\\pa51.dat", "a+t")) == NULL)
 {
   fprintf(stderr, "Cannot open input file.\n");
   return 1;
 }
  ...
  for (j=0;j<4;j++)
   {
    for (k=0;k<3;k++)
    {
     bb=pow(10,(bbb[j]/10));
     kk=kkk[k];
     fprintf(out0,"\n\n --- Rician pdf for K=%4.2lf partial coherent : a=Eb_N0+%5.2lf dB ---",
	     kkk[k],bbb[j]);
     fprintf(out1,"\n\n --- Rician pdf for K=%4.2lf partial coherent : a=Eb_N0+%5.2lf dB ---"
	    ,kkk[k],bbb[j]);
     printf("\n\n --- Rician pdf for K=%4.2lf partial coherent : a=Eb_N0+%5.2lf dB ---"
     ...
     sum+=qromb( func, X_1, X_2  );   // cos(x) in Q(.) should be positive!
     fprintf(out0,"\nEb_N0=%7.3f dB        result=%12.7lf ",i/4.0,log10(sum));
     fprintf(out1,"\n%12.7lf ",log10(sum));
     printf("\nEb_N0=%7.3f dB       result=%12.7lf ",i/4.0,log10(sum));
    }
    //getch();
